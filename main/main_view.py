...
def create_main_window():
    ...
    design_engs_button = create_button_frame(button_frame, [("Design Engineers", lambda:None)])
    design_engs_button.grid(row=1, column=0, padx=10, pady=10, sticky="ew")

    sales_engs_button = create_button_frame(button_frame, [("Sales Engineers", lambda:None)])
    sales_engs_button.grid(row=2, column=0, padx=10, pady=10, sticky="ew")

    proj_managers_button = create_button_frame(button_frame, [("Project Managers", lambda:None)])
    proj_managers_button.grid(row=3, column=0, padx=10, pady=10, sticky="ew")

    client_button = create_button_frame(button_frame, [("Clients", lambda:None)])
    client_button.grid(row=0, column=1, padx=10, pady=10, sticky="ew")

    mech_cons_button = create_button_frame(button_frame, [("Mechanical Contractors", lambda:None)])
    mech_cons_button.grid(row=1, column=1, padx=10, pady=10, sticky="ew")

    mech_engs_button = create_button_frame(button_frame, [("Mechanical Engineers", lambda:None)])
