...
###############################################################
    ...
    def _create_project_number_frame(self):
        ...
        self.project_number_frame = ttk.LabelFrame(self.base_frame, text='Project EM')
        self.project_number_frame.grid(row=0, column=0, padx = (10,0), pady = (10,0), sticky='w')

        self.combo_project_number = tk.StringVar()
        self.project_combo = ttk.Combobox(self.project_number_frame,
    ...
    def create_entry_widget_frame(self, parent):
        '''Creates entry dwgno label, entry widget, diagram & system dropdown'''
        ...
        def create_entry_widget(parent, entry_count, in_column, in_row):
        ...
        entry_label.grid(row=1, column=0, padx=(0,5))

        # Create title entry widget
        entry_widget = create_entry_widget(entry_frame)

        # Create diagram label
        diagram_label = ttk.Label(entry_frame, text='Diagram', font=('Helvetica', 9, 'bold'))
        diagram_label.grid(row=0, column=2, padx=(0,0))

        # Create Diagram combobox
        diagram_combo = ttk.Combobox(entry_frame)
        diagram_combo.grid(row=1, column=2, padx=(10,0), sticky="nsew")

        # Create system label
        system_label = ttk.Label(entry_frame, text='System', font=('Helvetica', 9, 'bold'))
        system_label.grid(row=0, column=3, padx=(0,0))

        # Create System combobox
        system_combo = ttk.Combobox(entry_frame)
        system_combo.grid(row=1, column=3, padx=(10,0), sticky="nsew")
        ...
        def _load_project_titles(project_number):
            ...
            def _default_title_entries():
                ...
                for _ in range(self.title_column_break):
                    self.create_entry_widget_frame(self.titles_frame)
            ...
            if not title_objs_list:
                _default_title_entries()
            else:
                ...
                # Create entry_widget_column_frame
                for 

                for title_obj in title_objs_list:
                    entry_widget = self.create_entry_widget_frame(self.titles_frame,
                                                                  entry_count, in_column, in_row)
