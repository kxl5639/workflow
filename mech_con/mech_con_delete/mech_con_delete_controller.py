...
def delete_selected_mech_cons(tree):
    ...
    mech_con_names = [f"{session.query(MechCon).get(item).first_name} {session.query(MechCon).get(item).last_name}" for item in selected_items]
    mech_con_names_str = "\n".join(mech_con_names)
    
    if len(selected_items) == 1:
        if show_custom_confirmation_message(tree, "Confirm Deletion", f"Are you sure you want to delete Sales engineer {mech_con_names[0]}?"):
            mech_con_id = selected_items[0]
            mech_con = session.query(MechCon).get(mech_con_id)
            error_message = delete_mech_con(mech_con)
            if error_message:
                show_custom_error_message(tree, "Error", f"Error deleting Sales engineer {mech_con_names[0]}: {error_message}")
            else:
        ...
        if show_custom_confirmation_message(tree, "Confirm Deletion", f"Confirm you want to delete Sales engineers:\n\n{mech_con_names_str}"):
            if show_custom_confirmation_message(tree, "Confirm Deletion", f"FINAL warning! This cannot be undone.\n\nPlease confirm you want to delete Sales engineers:\n\n{mech_con_names_str}"):
                for mech_con_id in selected_items:
                    ...
                    error_message = delete_mech_con(mech_con)
                    if error_message:
                        show_custom_error_message(tree, "Error", f"Error deleting Sales engineer {mech_con_names[mech_con_names.index(f'{mech_con.first_name} {mech_con.last_name}')]}: {error_message}")
                        break
                refresh_table(tree, MechCon, session, field_metadata)
